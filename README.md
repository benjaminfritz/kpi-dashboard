...
- `GET /api/dashboard-timeseries?span=30d|90d|365d&month_offset=N`: load daily trend series for charting (N=0 current window, 1 previous month window, etc.)
...
`vercel.json` schedules `/api/cron-dashboard-snapshot` once per week on Mondays at `06:00` UTC. Vercel should call that route with `Authorization: Bearer $CRON_SECRET`, and the handler will reject unauthenticated requests in production.
